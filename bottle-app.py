...
def report():
    ...
    c = conn.cursor()
# This code is convenient but need to teak the table.
# By adding an id header.
#   c.execute("SELECT * FROM ministryreport")
    c.execute("SELECT date, day, ministry, preacher, asst, message, att, tithes, offering, remarks FROM ministryreport")
    result = c.fetchall()
